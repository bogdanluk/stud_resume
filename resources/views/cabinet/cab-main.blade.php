...
@section('content')
    ...
    <main class="flex flex-col items-center w-full px-5 min-h-screen">
        ...
        @if(session('message'))
            <div class="flex mt-3 p-4 mb-4 text-green-800 border-t-4 border-green-400 bg-green-50 rounded-md dark:bg-slate-700">
                <p class="ml-1 text-sm font-medium">{{ session('message') }}</p>
            </div>
        @endif
        @if($errors->any())
            <div class="block mt-3 mb-3 p-2 text-red-800 border-t-4 border-red-400 bg-red-50 dark:bg-slate-700 rounded-md">
