    ...
    <div class="flex flex-col items-center w-full">
        ...
        <div class="flex flex-col items-center w-full">
            ...
            <div class="flex flex-row justify-center flex-wrap w-3/5 bg-base-200 border-t-8 border-t-violet-400 rounded-lg shadow-lg mb-10 dark:bg-slate-700">
                <div class="flex flex-col items-center my-5 mx-5">
                    <h2 class="font-medium pb-5">Компания</h2>
                    <div class="flex flex-col items-center">
                        <img class="max-h-96 w-28 rounded-lg" src="/storage/news/{{ $company->image }}" alt="{{ $company->image}}"/>
                        <p class="pt-5">{{$company->name}}</p>
                    </div>
                </div>
                <div class="flex flex-col items-center flex-wrap my-5 mx-5 col-span-3">
                    <h2 class="font-medium">Контактные данные</h2>
                    <div class="flex flex-row justify-center flex-wrap text-center my-5">
                        <div class="flex flex-col mx-5">
                            <span class="bg-yellow-400 rounded-full font-medium py-1 px-5 mt-5">Email</span></p>
                            <a class="my-2 hover:text-violet-400" href="mailto:{{$company->email}}">{{$company->email}}</a>
                        </div>
                        <div class="flex flex-col mx-5">
                            <span class="bg-yellow-400 rounded-full font-medium py-1 px-5 mt-5">Телефон</span></p>
                            <p class="my-2">{{$company->phone}}</p>
                        </div>
                        <div class="flex flex-col mx-5">
                            <span class="bg-yellow-400 rounded-full font-medium py-1 px-5 mt-5">Сайт</span></p>
                            <a class="my-2 hover:text-violet-400" href="{{$company->link}}">Перейти на сайт</a>
                        </div>
                    </div>
                    <a class="btn bg-violet-400 text-white font-medium w-2/4 ease-in hover:bg-violet-600" href="{{ route('company.open_post', $company->id) }}">Подробнее</a>
                </div>
            </div>
            ...
            <div class="flex flex-col w-11/12 bg-base-200 border-t-8 border-t-violet-400 rounded-lg shadow-lg dark:bg-slate-700">
                ...
                <pre class="font-sans pl-5 pt-3 whitespace-pre-wrap break-normal">{{$post->description}}</pre>
                <h2 class="pt-5 pl-5 font-medium">Обязанности работника</h2>
                <pre class="font-sans px-5 pt-3 whitespace-pre-wrap break-normal">{{$post->responsibility}}</pre>
                <h2 class="pt-5 pl-5 font-medium">Требования к работнику</h2>
                <pre class="font-sans px-5 pt-3 whitespace-pre-wrap break-normal">{{$post->requirement}}</pre>
                <h2 class="pt-5 pl-5 font-medium">Условия работы</h2>
                <pre class="font-sans px-5 pt-3 whitespace-pre-wrap break-normal">{{$post->conditions}}</pre>
                <h2 class="pt-5 pl-5 font-medium">Требуемые навыки</h2>
                <pre class="font-sans px-5 pt-3 whitespace-pre-wrap break-normal">{{$post->skills}}</pre>
