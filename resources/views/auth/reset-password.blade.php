        ...
        <div class="relative py-3 sm:w-96 mx-auto text-center">
            ...
            <div class="mt-4 bg-white shadow-lg rounded-lg text-left">
                ...
                <form class="px-8 py-6 " action="{{ route('password.update') }}" method="post">
                    @csrf
                    <input type="hidden" name="email" value="{{ $email }}" placeholder="Email" class="border w-full h-5 px-3 py-5 mt-2 focus:outline-none focus:ring-violet-400 focus:ring-1 rounded-md">
                    <label class="block font-semibold" for="input1">Введите новый пароль</label>
                    <input type="password" id="input1" name="password" placeholder="Новый пароль" class="border w-full h-5 px-3 py-5 mt-2 focus:outline-none focus:ring-violet-400 focus:ring-1 rounded-md">
                    <label class="block font-semibold" for="input2">Повторите пароль</label>
                    <input type="password" id="input2" name="password_confirmation" placeholder="Повторите пароль" class="border w-full h-5 px-3 py-5 mt-2 focus:outline-none focus:ring-violet-400 focus:ring-1 rounded-md">
